...
DROP PROCEDURE IF EXISTS `GetUnfinishedScanSummariesBySourceOrgID`;

CREATE PROCEDURE `GetUnfinishedScanSummariesBySourceOrgID` (_SourceID NVARCHAR(36), _OrgID NVARCHAR(36))
  #BEGIN#
  SELECT
    SS.SourceId,
    SS.TemplateId,
  ...
  WHERE SS.ScanStatus NOT IN ('finished','canceled','stopped','error')
        AND _SourceID = SS.SourceId AND _OrgID = SS.OrgId;